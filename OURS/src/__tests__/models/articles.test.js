 ...
 * Tests
 */
it('Articles fetchList() returns correctly', () => {
  ...
  model.effects(dispatch).fetchList({ page: 2 }, initialState).then((res) => {
...
it('Articles fetchList() does not go to API if lastSync just set', () => {
  const initialState = { articles: { lastSync: { 2: moment() } } };
  model.effects().fetchList({ page: 2 }, initialState).then((res) => expect(res).toEqual(true));
});

it('Articles fetchSingle() returns correctly', () => {
  Api.get.mockResolvedValue({ data: mockInput });

  model.effects().fetchSingle(222).then((res) => {
